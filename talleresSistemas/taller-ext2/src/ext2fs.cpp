...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned int cantDirABloqxBloq = block_size / sizeof(unsigned int);
	unsigned int res;
	if (block_number < 12){
		res = inode->block[block_number];
	}else if(block_number < cantDirABloqxBloq + 12){
		// 12 < block_number  < cantDirABloqxBloq + 12

		//asumiendo buffer arreglo de unsigned int
		unsigned int buffer[block_size];
		read_block(inode->block[12],buffer);
		res = buffer[block_number-12];


	} else if (block_number < cantDirABloqxBloq*cantDirABloqxBloq + 12 + cantDirABloqxBloq){
		unsigned int bloque = (block_number-12-cantDirABloqxBloq)/cantDirABloqxBloq;
		unsigned int buffer[block_size];
		unsigned int buffer2[block_size];
		read_block(inode->block[13],buffer);
		read_block(buffer[bloque],buffer2);
		res = buffer2[(block_number-12-cantDirABloqxBloq) % cantDirABloqxBloq];
	} else {
		//
		unsigned int bloque = (block_number-12-cantDirABloqxBloq-cantDirABloqxBloq*cantDirABloqxBloq)/(cantDirABloqxBloq*cantDirABloqxBloq);
		unsigned int bloque2 = (block_number-12-cantDirABloqxBloq-cantDirABloqxBloq*cantDirABloqxBloq)/cantDirABloqxBloq;
		unsigned int buffer[block_size];
		unsigned int buffer2[block_size];
		unsigned int buffer3[block_size];
		read_block(inode->block[14],buffer);
		read_block(buffer[bloque],buffer2);
		read_block(buffer2[bloque2],buffer);
		res = buffer3[block_number-12-cantDirABloqxBloq-cantDirABloqxBloq*cantDirABloqxBloq];
